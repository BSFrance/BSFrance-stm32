...
{										   
	ILI9348_WR_REG(Reg);		//写入要读的寄存器序号
	delayMicroseconds(5);		  
	return ILI9348_RD_DATA();		//返回读到的值
}   
//开始写GRAM
...
{							    
	ILI9348_WR_DATA(RGB_Code);//写十六位GRAM
}
...
uint16_t ILI9348_ReadPixel(uint16_t x,uint16_t y)
{
	...
	ILI9348_WR_REG(0X2E);//9348/6804/3510 发送读GRAM指令
	r=ILI9348_RD_DATA();							//dummy Read	   
	opt_delay(2);	  
 	r=ILI9348_RD_DATA();  		  						//实际坐标颜色
	opt_delay(2);	  
	b=ILI9348_RD_DATA(); 
	g=r&0XFF;		//对于9348/5310/5510,第一次读取的是RG的值,R在前,G在后,各占8位
	g<<=8;
	return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9348/NT35310/NT35510需要公式转换一下
}			 
//LCD开启显示
void ILI9348_DisplayOn(void)
{					   
	ILI9348_WR_REG(0X29);	//开启显示
}	 
//LCD关闭显示
void ILI9348_DisplayOff(void)
{	   
	ILI9348_WR_REG(0X28);	//关闭显示
}   
//设置光标位置
//Xpos:横坐标
//Ypos:纵坐标
...
//设置LCD的自动扫描方向
//注意:其他函数可能会受到此函数设置的影响(尤其是9348/6804这两个奇葩),
//所以,一般设置为L2R_U2D即可,如果设置为其他扫描方式,可能导致显示不正常.
//dir:0~7,代表8个方向(具体定义见lcd.h)
//9320/9325/9328/4531/4535/1505/b505/8989/5408/9348/5310/5510等IC已经实际测试	   	   
...
{
	ILI9348_SetCursor(x,y);		//设置光标位置 
	ILI9348_WriteRAM_Prepare();	//开始写入GRAM
...
void ILI9348_Display_Dir(uint8_t dir)
{
	...
	ILI9348_Scan_Dir(DFT_SCAN_DIR);	//默认扫描方向
}	 
...
//初始化lcd
//该初始化函数可以初始化各种ILI9348液晶,但是其他函数是基于ILI9320的!!!
//在其他型号的驱动芯片上没有测试! 
...
void ILI9348_Clear(uint16_t color)
{
    ...
    ILI9348_SetCursor(0x00,0x0000);	//设置光标位置 
...
{          
	...
	uint16_t xlen=0;
	{
		...
		for(i=sy;i<=ey;i++)
		{
		 	ILI9348_SetCursor(sx,i);      				//设置光标位置 
			ILI9348_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++)ILI9348_WR_DATA(color);	//设置光标位置 	    
		}
	}	 
}  
...
{  
	...
	height=ey-sy+1;			//高度
 	for(i=0;i<height;i++)
	{
 		ILI9348_SetCursor(sx,sy+i);   	//设置光标位置 
		ILI9348_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)ILI9348_WR_DATA(color[i*height+j]);//写入数据 
	}	  
}  
