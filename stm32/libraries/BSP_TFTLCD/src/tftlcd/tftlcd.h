...
//LCD参数
extern LCD_DevTypeDef lcddev;	//管理LCD重要参数
extern LCD_DrvTypeDef* pLcdDrv; //驱动程序
//LCD的画笔颜色和背景色	   
...
void LCD_WritePixel(uint16_t x,uint16_t y,uint16_t color);											//画点
void LCD_Fast_WritePixel(uint16_t x,uint16_t y,uint16_t color);								//快速画点
uint16_t  LCD_ReadPixel(uint16_t x,uint16_t y); 											//读点 
...
void LCD_SetDisplayWindow(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height);//设置窗口
