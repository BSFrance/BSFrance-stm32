...
#else
# error "un support! please add me"
#endif
...
#define GPIO_PORT_DATA_R  GPIOA->IDR
...
#else
#error "un support mcu please add me"
#endif
...
void dosampling() {
  uint32_t i = 0x4ffffff;  // 10s count
  ...
  switch (divider) {
    case 3:   //100m/23m -1 =3
      LL_SYSTICK_DisableIT();/*systick_disable(); //2680us */
      do {
        logicdata[i--] = *port; //1
        logicdata[i--] = *port; //2
        logicdata[i--] = *port; //3
      ...
      LL_SYSTICK_EnableIT();  /*systick_enable();*/
      break;
    case 9: // 100/(9+1) = 10M
      do {
        asm volatile("nop");
