...
void OLED_I2C::_convert_float(char *buf, double num, int width, byte prec){
  ...
  sprintf(buf, "%d.%02d", (int)num, (int)(num*100)%100);

	//sprintf(buf, "10.5");
	//char format[20];
  //sprintf(format, "%%%d.%df", width, prec);
  //sprintf(buf, format, num);
	//sprintf(buf, "%s", String(num).c_str());
	//String sf(f, 5);
}
...
void OLED_I2C::_sendTWIcommand(uint8_t value)
{
	...
	if (_use_hw)
	{
    HWIRE.beginTransmission(SSD1306_ADDR);//SSD1306_ADDR<<1
    WIRE_WRITE(SSD1306_COMMAND); //0x00
    WIRE_WRITE(value);
    HWIRE.endTransmission();
